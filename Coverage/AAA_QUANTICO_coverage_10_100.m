...
filename = ['QR_Y_sim1_TRUEs_',num2str(sample_size),'.csv'];
QR_Y_sim_TRUEs = csvread(filename);
no_simulations = 50;

Summary = cell(no_simulations,1);
...
for zzzz = 1:no_simulations
    ...
    %QR_Y_sim_TRUEs = (QR_Y_sim_TRUEs_raw - min(y_raw))/(max(y_raw)-min(y_raw));
    ...
    a_tau = 5; b_tau = 25; %use (5,25) for real data (5,100) for simulation
    s0 = 2.5*10^-4;
    %a_w = 1/2;b_w=1/2;
...
%         end
    ...
    TPR_P_AUC = zeros(length(ts),length(cutoffs));
    FPR_P_AUC = zeros(length(ts),length(cutoffs));
    
    TP_G_FDR_AUC = zeros(length(ts),length(cutoffs));
    FP_G_FDR_AUC = zeros(length(ts),length(cutoffs));
    TPR_G_FDR_AUC = zeros(length(ts),length(cutoffs));
    FPR_G_FDR_AUC = zeros(length(ts),length(cutoffs));
    ...
    AUC_P = zeros(length(ts),1);
    AUC_G = zeros(length(ts),1);
    ...
    for jjj = 1:length(ts)
        ...
        for cut = 1:length(cutoffs)
            ...
            TPR_P_AUC(jjj,cut) = TP_P_AUC(jjj,cut)/(n*max(size(true_coords_P{jjj})));
            FP_P_AUC(jjj,cut) = sum(sum(Prob_of_one{jjj}(:,null_coords_P{jjj})>cutoffs(cut)));
            FPR_P_AUC(jjj,cut) = FP_P_AUC(jjj,cut)/(n*max(size(null_coords_P{jjj})));
            %%%% G variable TP/FP %%%%
            ...
            TPR_G_FDR_AUC(jjj,cut) = TP_G_FDR_AUC(jjj,cut)/total_non_zero_G;
            FP_G_FDR_AUC(jjj,cut) = sum(sum(selected_G_AUC)) - TP_G_FDR_AUC(jjj,cut);
            FPR_G_FDR_AUC(jjj,cut) = FP_G_FDR_AUC(jjj,cut)/(g*(p-1) - total_non_zero_G);
        end
        
        %%%% AUC  P and G %%%%% 
        
        TPR_FPR_P_AUC = transpose([TPR_P_AUC(jjj,:);FPR_P_AUC(jjj,:)]);
        uniques_TPR_FPR_P = unique(TPR_FPR_P_AUC,'rows');
        
        TPR_FPR_G_AUC = transpose([TPR_G_FDR_AUC(jjj,:);FPR_G_FDR_AUC(jjj,:)]);
        uniques_TPR_FPR_G = unique(TPR_FPR_G_AUC,'rows');
            
        
        
        [aaaa,bbbb] = sort(uniques_TPR_FPR_P(:,2));
        sorted_mat_P = zeros(max(size(uniques_TPR_FPR_P)),2);
        sorted_mat_P(:,2) = aaaa;
        sorted_mat_P(:,1) = uniques_TPR_FPR_P(bbbb,1);
        AUC_P(jjj) = trapz(sorted_mat_P(:,2),sorted_mat_P(:,1));
        
        [cccc,dddd] = sort(uniques_TPR_FPR_G(:,2));
        sorted_mat_G = zeros(max(size(uniques_TPR_FPR_G)),2);
        sorted_mat_G(:,2) = cccc;
        sorted_mat_G(:,1) = uniques_TPR_FPR_G(dddd,1);
        AUC_G(jjj) = trapz(sorted_mat_G(:,2),sorted_mat_G(:,1));
    ...
    patient_TRUE_vs_ESTIMATE_diff = abs(QR_Y_sim_TRUEs(:,which_quaniles) - QR_Y_sim_ESTs(:,which_quaniles));
    DIFFS =  max(patient_TRUE_vs_ESTIMATE_diff,[],2);
    
    expanded_width_1 = 0.8*sqrt(log(n))*width;
    expanded_width_2 = 1*sqrt(log(n))*width;
    expanded_width_3 = 1*sqrt(log(n))*width;
    expanded_width_4 = 1.5*sqrt(log(n))*width;
    expanded_width_5 = 2*sqrt(log(n))*width;
    expanded_width_6 = 2.2*sqrt(log(n))*width;
    expanded_width_7 = 2.4*sqrt(log(n))*width;
    coverage = sum(width > DIFFS)/n;
    expanded_coverage_1 = sum(expanded_width_1 > DIFFS)/n;
    expanded_coverage_2 = sum(expanded_width_2 > DIFFS)/n;
    expanded_coverage_3 = sum(expanded_width_3 > DIFFS)/n;
    expanded_coverage_4 = sum(expanded_width_4 > DIFFS)/n;
    expanded_coverage_5 = sum(expanded_width_5 > DIFFS)/n;
    expanded_coverage_6 = sum(expanded_width_6 > DIFFS)/n;
    expanded_coverage_7 = sum(expanded_width_7 > DIFFS)/n;
    
    
    All_coverage(zzzz,:) = [coverage, expanded_coverage_1,expanded_coverage_2,expanded_coverage_3,...
        expanded_coverage_4,expanded_coverage_5,expanded_coverage_6,expanded_coverage_7];
    

    MSE = sum((QR_Y_sim_TRUEs - QR_Y_sim_ESTs).^2,1)/n;
    ...
    Summary{zzzz} = [transpose([TPR_P,FPR_P]);TPR_FPR_FDR;Total_TPR_FPR;...
        transpose(AUC_P);transpose(AUC_G);transpose(AUC);MSE];
    
end

Summary_3d = zeros(size(Summary{1},1),size(Summary{1},2),no_simulations);

for zzzz=1:no_simulations
    Summary_3d(:,:,zzzz) = Summary{zzzz};
end
mean_summary = mean(Summary_3d,3);
sd_summary = zeros(size(Summary{1},1),size(Summary{1},2));

for ii = 1:size(Summary{1},1)
    for jj = 1:size(Summary{1},2)
        elements = Summary_3d(ii,jj,1:no_simulations);
        sd_summary(ii,jj) = std(elements);
    end
end


%%%% Appended summary to save 2D
Append_summary = Summary{1};
for zzzz = 2:no_simulations
    Append_summary = [Append_summary,Summary{zzzz}];
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

All_coverage;
time = toc
filename_output = ['ZZZ_All_coverage_',num2str(num_of_p),'_samplesize_',num2str(sample_size),'_rep_',num2str(no_simulations),'_iters_',num2str(N),'.csv'];
csvwrite(filename_output, All_coverage);
filename_output = ['ZZZ_Summary_',num2str(num_of_p),'_samplesize_',num2str(sample_size),'_rep_',num2str(no_simulations),'_iters_',num2str(N),'.csv'];
csvwrite(filename_output, Append_summary);
filename_output = ['ZZZ_Mean_summary_',num2str(num_of_p),'_samplesize_',num2str(sample_size),'_rep_',num2str(no_simulations),'_iters_',num2str(N),'.csv'];
csvwrite(filename_output, mean_summary);
filename_output = ['ZZZ_Sd_summary_',num2str(num_of_p),'_samplesize_',num2str(sample_size),'_rep_',num2str(no_simulations),'_iters_',num2str(N),'.csv'];
csvwrite(filename_output, sd_summary);