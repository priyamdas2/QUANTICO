...
%     end
    ...
    % categ_est = cell(p,1);
    %
    % lin_COEFF_est = cell(p,1);
    % nlin_COEFF_est = cell(p,1);
    % categ_COEFF_est = cell(p,1);
    %
    % for j = 1:p
    ...
    %
    %     % Coefficients
    %     lin_COEFF_est{j} = mean(betax{j},2);
    %     nlin_COEFF_est{j} = mean(betaX{j},2);
    %     categ_COEFF_est{j} = mean(betaZ{j},2);
    % end
    % ln_COEFF_est = mean(betamu,2);
    %
    ...
    %
    %
    % est_1 = sum(u.*mean(v0,3),2);
    % est_2 = sum(u.*mean(v,3),2);
    %
    % error_1 = sum(abs(y - est_1));
    % error_2 = sum(abs(y - est_2));
